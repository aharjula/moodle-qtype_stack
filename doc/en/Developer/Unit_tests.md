...
Our intention is to conform to Moodle's code style guidelines.  To help achieve this we use the moodle codechecker plugin, which can be installed by the moodle database.  This section records some notes.

* STACK makes use of parsers which are the result of autogenerated code.  These do not (and cannot) conform to Moodle's coding style.  Therefore, when running the codecheker you need to exclude these files.

    */yui/build/*, */tests/fixtures/*, thirdparty/php-peg/*, castext/castextparser.class.php
